...
void Matmul(const M1Array& a, const M1Array& b, M1Array* out, uint32_t row_dim_x, uint32_t inner_dim, uint32_t col_dim_x) {
  ...
  } else if ((row_dim_x & 7) == 0 && (col_dim_x & 3) == 0 && (inner_dim & 3) == 0) {
    MetalOps->MatMul(a.array_MTL, b.array_MTL, out->array_MTL, {row_dim_x, col_dim_x, inner_dim, 16, 8, 4, 8}, "gemm_tiling");
  } else {
  ...
  // if ((row_dim_x & 7) == 0 && (col_dim_x & 3) == 0 && (inner_dim & 3) == 0) {
  //   MetalOps->MatMul(a.array_MTL, b.array_MTL, out->array_MTL, {row_dim_x, col_dim_x, inner_dim, 16, 8, 4, 8}, "gemm_tiling");
  // } else {

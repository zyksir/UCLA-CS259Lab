...
template<const uint BX, const uint BY, const uint BZ, 
    ...
    >
__global__ void gemm_block_tiling_kernel(const float *A, const float *B, float *C, const uint N, const uint K, const uint M) {
...
template<const uint N, const uint K, const uint M, 
  ...
  const uint TX, const uint TY>
void gemm_block_tiling(const float *A, const float *B, float *C, dim3 &grid, dim3 &block) {
	static_assert(K % BZ == 0 && N % BX == 0 && M % BY == 0, "");
    block = dim3(BX*BY/(TX*TY));
    grid = dim3(CEIL_DIV(N, BX), CEIL_DIV(M, BY));
    gemm_block_tiling_kernel<BX, BY, BZ, TX, TY>

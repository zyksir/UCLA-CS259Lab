...
{
    _mat_multiply_blas(A, B, C, alpha, beta);
}
...
{
    _mat_multiply_blas(A, B, C, 1.0f, 0.0f);
}
...
float assert_almost_equal_max_error(const MatrixF &X, const MatrixF &X_true, float tolerance) {
    float max_error = 0;
    for (int i = 0; i < X.size(); ++i)
    {
        const float error = std::abs(X[i] - X_true[i]);
        if (error > max_error) {
            max_error = error;
        }
        if (error > tolerance) {
